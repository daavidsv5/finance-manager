...
const EDU_TYPES = ['Kurz', 'Webinář', 'Kniha', 'Software', 'Ostatní'] as const;
type EduType = typeof EDU_TYPES[number];

const TYPE_STYLE: Record<string, string> = {
  Kurz:     'bg-blue-50 text-blue-700 ring-blue-200',
  Webinář:  'bg-violet-50 text-violet-700 ring-violet-200',
  Kniha:    'bg-amber-50 text-amber-700 ring-amber-200',
  Software: 'bg-emerald-50 text-emerald-700 ring-emerald-200',
...
export default function EducationPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filter !== 'all') d = d.filter(e => e.type === filter);
    d.sort((a, b) => {
    ...
    return d;
  }, [state.educationExpenses, filter, sortKey, sortDir]);

  const total = useMemo(() => filtered.reduce((s, e) => s + e.amount, 0), [filtered]);
  const grandTotal = useMemo(() => state.educationExpenses.reduce((s, e) => s + e.amount, 0), [state.educationExpenses]);
  ...
  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault();
    addEducationExpense({ name: form.name, type: form.type, paidBy: form.paidBy, amount: parseInt(form.amount), date: form.date });
    setForm({ name: '', type: 'Kurz', paidBy: 'David', amount: '', date: new Date().toISOString().split('T')[0] });
    setOpen(false);
  };
    ...
    <div className="space-y-7">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-[28px] font-bold text-[#0f1d3a]">Vzdělávání</h1>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-1">
        <SummaryCard title="Celkové vzdělávání" value={formatCzk(grandTotal)} icon={GraduationCap} variant="default" description={`${state.educationExpenses.length} položek celkem`} />
      </div>
      ...
      <div className="rounded-xl border border-[#dde5f4] bg-white shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#eef2fb]">
          <p className="text-[16px] font-bold text-[#0f1d3a]">Přehled vzdělávání</p>
          <Select value={filter} onValueChange={v => setFilter(v ?? 'all')}>
            <SelectTrigger className="h-9 w-40 text-[14px] border-[#dde5f4] bg-white text-[#0f1d3a] font-medium">
              <SelectValue placeholder="Typ" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">Všechny typy</SelectItem>
              {EDU_TYPES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr style={{ background: '#152347' }}>
                {[['name','Název',false],['type','Typ',false],['paidBy','Kdo platil',false],['date','Datum',false],['amount','Částka',true]].map(([k,lbl,r]) => (
            ...
            <tbody>
              {filtered.map((item, i) => (
                <tr key={item.id} className={`group border-b border-[#eef2fb] hover:bg-blue-50/50 transition-colors ${i % 2 === 1 ? 'bg-[#f8fafd]' : 'bg-white'}`}>
                  <td className="px-6 py-3.5 text-[15px] font-semibold text-[#0f1d3a] max-w-xs">{item.name}</td>
                  <td className="px-6 py-3.5">
                    <span className={`inline-flex items-center rounded-lg px-2.5 py-1 text-[12px] font-bold ring-1 ring-inset ${TYPE_STYLE[item.type] ?? 'bg-slate-100 text-slate-600 ring-slate-200'}`}>{item.type}</span>
                  </td>
                  <td className="px-6 py-3.5 text-[15px] text-slate-600 font-medium">{item.paidBy}</td>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-sm bg-white border-[#dde5f4]">
          <DialogHeader><DialogTitle className="text-[18px] font-bold text-[#0f1d3a]">Nový výdaj na vzdělávání</DialogTitle></DialogHeader>
          <form onSubmit={handleAdd} className="space-y-4 pt-2">
            <div className="space-y-2"><Label className="text-[14px] font-semibold text-slate-600">Název</Label><Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Název kurzu / webináře" className="h-10 text-[15px]" required /></div>
            <div className="space-y-2"><Label className="text-[14px] font-semibold text-slate-600">Typ</Label>
              <Select value={form.type} onValueChange={v => setForm(f => ({ ...f, type: (v ?? f.type) as EduType }))}>
                <SelectTrigger className="h-10 text-[15px]"><SelectValue /></SelectTrigger>
                <SelectContent>{EDU_TYPES.map(t => <SelectItem key={t} value={t}>{t}</SelectItem>)}</SelectContent>
              </Select>
            </div>
            <div className="space-y-2"><Label className="text-[14px] font-semibold text-slate-600">Kdo platil</Label><Input value={form.paidBy} onChange={e => setForm(f => ({ ...f, paidBy: e.target.value }))} placeholder="David" className="h-10 text-[15px]" required /></div>
