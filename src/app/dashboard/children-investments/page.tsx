...
const INV_TYPES = ['Spoření', 'ETF', 'Akcie', 'Dluhopisy', 'Ostatní'];
const YEARS = Array.from({ length: 8 }, (_, i) => 2020 + i);

const TYPE_STYLE: Record<string, string> = {
  Spoření:    'bg-pink-50 text-pink-700 ring-pink-200',
...
export default function ChildrenInvestmentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return d;
  }, [investments, fYear, fType, sortKey, sortDir]);

  const total = useMemo(() => filtered.reduce((s, i) => s + i.amount, 0), [filtered]);
  const grandTotal = useMemo(() => investments.reduce((s, i) => s + i.amount, 0), [investments]);
  ...
  const handleAdd = (e: React.FormEvent) => {
    e.preventDefault();
    addChildInvestment({ year: form.year, date: form.date, amount: parseInt(form.amount), description: form.description, type: form.type });
    setForm({ year: new Date().getFullYear(), date: new Date().toISOString().split('T')[0], amount: '', description: '', type: 'Spoření' });
    setOpen(false);
  };
    ...
    <div className="space-y-7">
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-[28px] font-bold text-[#0f1d3a]">Děti investice</h1>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-1">
        <SummaryCard title="Celkové investice" value={formatCzk(grandTotal)} icon={TrendingUp} variant="default" description={`${investments.length} položek celkem`} />
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr style={{ background: '#152347' }}>
                  {[['year','Rok',false],['date','Datum',false],['type','Typ',false],['amount','Částka',true]].map(([k,lbl,r]) => (
